...
const checkIfFavorite = (track: ITrack, userId: string | null): boolean => {
...
function MusicItem({ track, trackNumber }: MusicItemProps) {
  ...
  const computedIsFavorite = useMemo(
    () => checkIfFavorite(track, userId),
    [track, userId]
  )

  const [localFavoriteState, setLocalFavoriteState] = useState<boolean | null>(
    null
  )

  const hasSpotifyUrl =
    track.external_urls?.spotify && track.external_urls.spotify.trim() !== ''
  const hasDuration = !!(track.duration_ms && Number(track.duration_ms) > 0)

  const isFavorite =
    localFavoriteState !== null ? localFavoriteState : computedIsFavorite

  useEffect(() => {
    const handleFavoriteUpdate = () => {
      const newFavoriteState = checkIfFavorite(track, userId)
      setLocalFavoriteState(newFavoriteState)
    }

    window.addEventListener('favoriteMusicsUpdated', handleFavoriteUpdate)

    return () => {
      window.removeEventListener('favoriteMusicsUpdated', handleFavoriteUpdate)
    }
  }, [track, userId])
  ...
  const handleToggleFavorite = (e: React.MouseEvent) => {
    ...
    const savedMusics = localStorage.getItem(storageKey)
    let favoriteMusics: FavoriteMusic[] = savedMusics
      ? JSON.parse(savedMusics)
      : []
