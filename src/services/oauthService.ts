  ...
  if (!isValidState) {
    ...
    if (!state) {
      ...
      if (savedState) {
        ...
        sessionStorage.removeItem('spotify_oauth_state')
        throw new Error(
          'State não recebido. Por segurança, faça login novamente.'
        )
      }
      ...
      if (!code) {
        throw new Error(
          'Código de autorização não encontrado. Faça login novamente.'
        )
      }
      // Se há código mas não há state, pode ser um caso edge
      // Por segurança, não permitimos
      throw new Error(
        'State não encontrado. Por segurança, faça login novamente.'
      )
    } else {
