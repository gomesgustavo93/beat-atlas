...
const Favorites = () => {
  ...
  const {
    ...
    error: artistsError,
  } = useSpotifyTopArtists(timeRange, ITEMS_PER_PAGE, offset)
  const {
    data: tracksData,
    isLoading: isLoadingTracks,
    error: tracksError,
  } = useSpotifyTopTracks(timeRange, ITEMS_PER_PAGE, offset)

  const { data: allTracksData } = useSpotifyTopTracks(timeRange, 50, 0)

  const handleTypeChange = (newType: 'artists' | 'tracks') => {
    setType(newType)
    setCurrentPage(1)
  }
  ...
  const totalPages =
    type === 'artists'
      ? artistsData?.total
        ? Math.ceil(artistsData.total / ITEMS_PER_PAGE)
        : 1
      : tracksData?.total
        ? Math.ceil(tracksData.total / ITEMS_PER_PAGE)
        : 1

  const total =
    type === 'artists' ? artistsData?.total || 0 : tracksData?.total || 0
        ...
        <div className={cn('flex', 'gap-4', 'flex-wrap')}>
          <FormControl sx={{ minWidth: 200 }}>
            ...
            <Select
              ...
              value={type}
              onChange={e =>
                handleTypeChange(e.target.value as 'artists' | 'tracks')
              }
          ...
          <FormControl sx={{ minWidth: 300 }}>
            ...
            >
              <MenuItem value="short_term">
                {t('favorites.timeRange.4weeks')}
              </MenuItem>
              <MenuItem value="medium_term">
                {t('favorites.timeRange.6months')}
              </MenuItem>
              <MenuItem value="long_term">
                {t('favorites.timeRange.allTime')}
              </MenuItem>
            </Select>
          </FormControl>
        </div>
