...
const createFavoriteMusicSchema = (t: (key: string) => string) =>
  z.object({
    name: z
      .string()
...
interface FavoriteMusic extends Omit<
  ITrack,
  'id' | 'album' | 'preview_url' | 'popularity'
> {
  id: string
  album: {
    name: string
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener(
        'favoriteMusicsUpdated',
        handleFavoriteMusicsUpdate
      )
    }
  }, [loadFavoriteMusics])
  ...
  const onSubmit = (data: FavoriteMusicForm) => {
    ...
    const newMusic: FavoriteMusic = selectedTrack
      ? {
          ...
          external_urls: {
            spotify:
              data.spotifyUrl || selectedTrack.external_urls?.spotify || '',
          },
          duration_ms:
            parseDuration(data.duration) || selectedTrack.duration_ms || 0,
        }
      : {
    ...
    <Container>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              {errors.spotifyUrl ? (
                ...
                <p className="mt-1 text-sm text-gray-500">
                  {t('myMusics.form.spotifyUrlHint')}
                </p>
              )}
            </div>
      ...
      <div>
        ...
        {favoriteMusics.length === 0 ? (
          <p className={cn('text-center', 'text-gray-600', 'py-10')}>
            {t('myMusics.noMusics')}
          </p>
        ) : (
