...
function Home() {
    ...
    const { data: artistsData, isLoading: isLoadingArtists, error: artistsError } = useSpotifySearch(
        debouncedQuery,
        ['artist'],
    ...
    const { data: tracksData, isLoading: isLoadingTracks, error: tracksError } = useSpotifySearch(
        debouncedQuery,
        ['track'],
        SEARCH_ITEMS_PER_PAGE,
        tracksOffset,
    ...
    const { data: albumsData, isLoading: isLoadingAlbums, error: albumsError } = useSpotifySearch(
        debouncedQuery,
        ['album'],
        SEARCH_ITEMS_PER_PAGE,
        albumsOffset,
                    ...
                    <div className="mb-0">
                        ...
                        {searchError && (
                            <div className={cn(
                                ...
                                <strong>{t('home.searchResults.error')}:</strong> {searchError.message}
                            </div>
                        )}

                        {!isSearchLoading && !searchError && searchData && (
                            <>
                                {selectedTypes.includes('artist') && searchData.artists && searchData.artists.items && searchData.artists.items.length > 0 && (
                                    <div className="mb-10">
                                        <h3 className={cn(
                                            'mb-4',
                                            'text-xl'
                                        )}>{t('home.searchResults.artists')}</h3>
                                        <div className={cn(
                                            'grid',
                                            'grid-cols-2',
                                            'sm:grid-cols-3',
                                        ...
                                        {searchData.artists.total > SEARCH_ITEMS_PER_PAGE && (
                                            <Pagination
                                                ...
                                                totalPages={Math.ceil(searchData.artists.total / SEARCH_ITEMS_PER_PAGE)}
                                                onPageChange={(page) => {
                                                    setArtistsPage(page);
                                ...
                                {selectedTypes.includes('track') && searchData.tracks && searchData.tracks.items && searchData.tracks.items.length > 0 && (
                                    <div className="mb-10">
                                        <h3 className={cn(
                                            'mb-4',
                                            'text-xl'
                                        )}>{t('home.searchResults.tracks')}</h3>
                                        <div className={cn(
                                            'flex',
                                            'flex-col',
                                            'gap-2.5',
                                            ...
                                            {searchData.tracks.items.map((track: ITrack, index: number) => {
                                        ...
                                        {searchData.tracks.total > SEARCH_ITEMS_PER_PAGE && (
                                            <Pagination
                                                ...
                                                totalPages={Math.ceil(searchData.tracks.total / SEARCH_ITEMS_PER_PAGE)}
                                                onPageChange={(page) => {
                                                    setTracksPage(page);
                                                    window.scrollTo({ top: 0, behavior: 'smooth' });
                                                }}
                                ...
                                {selectedTypes.includes('album') && searchData.albums && searchData.albums.items && searchData.albums.items.length > 0 && (
                                    <div className="mb-10">
                                        <h3 className={cn(
                                            'mb-4',
                                            'text-xl'
                                        )}>{t('home.searchResults.albums')}</h3>
                                        <div className={cn(
                                            'grid',
                                            'grid-cols-2',
                                            'sm:grid-cols-3',
                                        ...
                                        {searchData.albums.total > SEARCH_ITEMS_PER_PAGE && (
                                            <Pagination
                                                ...
                                                totalPages={Math.ceil(searchData.albums.total / SEARCH_ITEMS_PER_PAGE)}
                                                onPageChange={(page) => {
                                                    setAlbumsPage(page);
                                                    window.scrollTo({ top: 0, behavior: 'smooth' });
                                                }}
