...
import { getUserAccessToken as getUserAccessTokenService, logout as logoutService } from "../../services/oauthService";
...
userHttpClient.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (error.response?.status === 403) {
      const errorMessage = (error.response.data as any)?.error?.message || 'Insufficient client scope';
      if (errorMessage.includes('scope') || errorMessage.includes('Insufficient')) {
        logoutService();
